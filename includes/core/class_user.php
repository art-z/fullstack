...
class User
{
    ...
    {
        $q = DB::query("SELECT user_id, access, first_name, last_name, phone, email, plot_id
    ...
    public static function users_list($d = [])
    {
        ...
        // where
        $where = [];
        if ($search) $where[] = "phone LIKE '%" . $search . "%' OR 
    ...
    public static function user_edit_update($d = [])
    {
        ...
        $user_id = isset($d['user_id']) ? $d['user_id'] : false;
        $offset = isset($d['offset']) ? preg_replace('~\D+~', '', $d['offset']) : 0;
        //test if many plots
        $plots = isset($d['plot_id']) ? explode(',', $d['plot_id']) : [];

        $plot_ids = [];
        foreach ($plots as $plot_id) {
            if ((int)(trim($plot_id))) {
                $plot_ids[] = (int)(trim($plot_id));
            }
        }
        $plot_str = implode(',', $plot_ids);
    ...
    public static function users_fetch($d = [])
    {
        ...
        HTML::assign('paginator', $info['paginator']);

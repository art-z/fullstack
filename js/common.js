let common = {
    ...
    user_edit_update: (user_id = 0) => {
        ...
        let warns = qs_all('.warn');
        for(let i = 0; i < warns.length; i++){
            remove_class(warns[i], 'warn');
        }

        if (!data.first_name || !data.last_name || !validatePhone(data.phone) || !validateEmail(data.email)) {
            if(!data.first_name){
                add_class('first_name', 'warn');
            }
            if(!data.last_name){
                add_class('last_name', 'warn');
            }
            if(!validatePhone(data.phone)){
                add_class('phone', 'warn');
            }
            if(!validateEmail(data.email)){
                add_class('email', 'warn');
            }
